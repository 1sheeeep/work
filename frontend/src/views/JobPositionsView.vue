...
function captureLabel(job: JobPosition) {
  ...
  return job.captureSource === 'VISIBLE_PAGE' ? `页面采集${job.captureCompleteness ? ` · ${job.captureCompleteness}/6` : ''} · ${job.captureVerified ? '已核对' : '待核对'}` : '手工录入'
}
...
function realValue(value?: string) { return value?.includes('待从 BOSS 岗位页补全') ? '' : value ?? '' }
function openImportedReview(job: JobPosition) {
  ...
  Object.assign(reviewForm, {
    ...
    screeningRequirements: realValue(job.screeningRequirements), replySummary: job.replySummary ?? '',
