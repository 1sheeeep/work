...
function openCreate() {
  ...
  Object.assign(form, { companyId: activeCompanies.value[0]?.id ?? '', bossAccountId: '', title: '', location: '', salaryMinK: 20, salaryMaxK: 30, salaryMonths: 13, experienceRequirement: '', educationRequirement: '', description: '', screeningRequirements: '' })
...
<template>
  <div class="page-shell">
    ...
    <template v-else>
      <div class="metrics-strip"><div><span>职位总数</span><strong>{{ stats.total }}</strong></div><div><span>已启用</span><strong>{{ stats.active }}</strong></div><div><span>待完善草稿</span><strong>{{ stats.draft }}</strong></div></div>
      <section class="surface-panel jobs-panel">
        ...
        <template v-else>
          <el-table :data="jobs" class="jobs-table"><el-table-column label="职位" min-width="210"><template #default="{ row }"><div class="job-identity"><strong>{{ row.title }}</strong><span>{{ row.location }} · {{ salaryLabel(row as JobPosition) }}</span></div></template></el-table-column><el-table-column label="归属企业" min-width="165"><template #default="{ row }"><strong>{{ row.company.name }}</strong><div class="muted">{{ row.company.code }}</div></template></el-table-column><el-table-column label="BOSS 账号" min-width="175"><template #default="{ row }"><strong>{{ row.bossAccount.displayName }}</strong><div class="muted">{{ row.bossAccount.externalIdentifier }}</div></template></el-table-column><el-table-column label="要求" min-width="160"><template #default="{ row }"><span>{{ row.experienceRequirement }}</span><div class="muted">{{ row.educationRequirement }}</div></template></el-table-column><el-table-column label="状态" width="100"><template #default="{ row }"><el-tag :type="statusTagType(row.status)">{{ statusLabels[row.status as JobPositionStatus] }}</el-tag></template></el-table-column><el-table-column v-if="canManage" label="操作" width="210" fixed="right"><template #default="{ row }"><el-button v-if="row.status !== 'CLOSED'" link type="primary" @click="openEdit(row as JobPosition)">编辑</el-button><el-button v-if="row.status === 'DRAFT'" link type="success" :loading="changingStatusId === row.id" @click="changeStatus(row as JobPosition, 'ACTIVE')">启用</el-button><el-button v-if="row.status !== 'CLOSED'" link type="danger" :loading="changingStatusId === row.id" @click="changeStatus(row as JobPosition, 'CLOSED')">关闭</el-button><span v-if="row.status === 'CLOSED'" class="muted">已终结</span></template></el-table-column></el-table>
          <div class="job-cards"><article v-for="job in jobs" :key="job.id"><header><div class="job-identity"><strong>{{ job.title }}</strong><span>{{ job.location }} · {{ salaryLabel(job) }}</span></div><el-tag :type="statusTagType(job.status)" size="small">{{ statusLabels[job.status] }}</el-tag></header><dl><div><dt>归属企业</dt><dd>{{ job.company.name }}</dd></div><div><dt>BOSS 账号</dt><dd>{{ job.bossAccount.displayName }}</dd></div><div><dt>经验 / 学历</dt><dd>{{ job.experienceRequirement }} · {{ job.educationRequirement }}</dd></div></dl><p class="job-description">{{ job.description }}</p><footer v-if="canManage && job.status !== 'CLOSED'"><el-button @click="openEdit(job)">编辑</el-button><el-button v-if="job.status === 'DRAFT'" type="success" plain :loading="changingStatusId === job.id" @click="changeStatus(job, 'ACTIVE')">启用</el-button><el-button type="danger" plain :loading="changingStatusId === job.id" @click="changeStatus(job, 'CLOSED')">关闭</el-button></footer></article></div>
        </template>
      </section>
    </template>
...
<style scoped>
.scope-alert{margin-bottom:20px}.metrics-strip{display:grid;grid-template-columns:repeat(3,1fr);margin-bottom:20px;border:1px solid var(--border);border-radius:12px;background:var(--surface);overflow:hidden}.metrics-strip div{padding:18px 24px;border-right:1px solid var(--border)}.metrics-strip div:last-child{border:0}.metrics-strip span,.metrics-strip strong{display:block}.metrics-strip span{color:var(--text-secondary);font-size:12px}.metrics-strip strong{margin-top:5px;font-size:24px}.jobs-panel{overflow:hidden}.jobs-title{align-items:flex-end}.filters{display:grid;grid-template-columns:minmax(220px,280px) 155px 125px auto;gap:8px}.jobs-table{width:100%}.job-identity strong,.job-identity span{display:block}.job-identity span,.muted{margin-top:4px;color:var(--text-secondary);font-size:12px}.job-cards{display:none}.dialog-alert{margin-bottom:18px}.form-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:0 18px}.form-grid .el-select,.form-grid .el-input-number{width:100%}.form-tip{margin-top:6px;font-size:12px;line-height:1.45}.form-tip.warning{color:var(--warning)}
