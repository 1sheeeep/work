  ...
  <div class="page-shell">
    ...
    <template v-else-if="summary">
      ...
      <section class="surface-panel safety-manifest">
        ...
        <div class="manifest-summary"><article><span>受控动作</span><strong>{{ connectorSafety.controlledActions }}</strong></article><article><span>故障场景</span><strong>{{ connectorSafety.faultScenarios.length }}</strong></article><article><span>账号隔离场景</span><strong>{{ connectorSafety.accountIsolationScenarios }}</strong></article><article><span>生产执行</span><strong class="healthy">{{ connectorSafety.productionEnabled ? '已启用' : '保持关闭' }}</strong></article></div>
