...
import type { BossAccount, BossAccountStatus, BossCapability, BossConnectionStatus, BossGatewayType, BrowserDevice, Company, MockBossProfile } from '../types'
...
function openEdit(account: BossAccount) {
  ...
  Object.assign(form, { companyId: account.company.id, displayName: account.displayName, externalIdentifier: account.externalIdentifier,gatewayType:account.gatewayType, mockProfile: account.mockProfile||'FULL' })
  formError.value = ''
  clearFieldErrors()
  dialogOpen.value = true
}
...
<template>
  <div class="page-shell">
    ...
    <template v-else>
      <section class="connect-panel"><div class="connect-heading"><div><h2>我的正式招聘账号</h2><p>只展示可连接真实 BOSS 页面的账号，Mock 数据不进入运行链路。</p></div><span>{{ connectableAccounts.length }} 个账号</span></div><div v-if="connectableAccounts.length === 0" class="surface-panel empty-state"><span class="empty-state__icon"><el-icon><Connection /></el-icon></span><strong>还没有可连接的正式招聘账号</strong><span>{{ canManage ? '请添加“正式浏览器招聘账号”，不要选择 Mock。' : '请联系招聘管理员为你分配正式账号。' }}</span><el-button v-if="canManage" type="primary" :icon="Plus" @click="openCreate">添加正式账号</el-button></div><div v-else class="connect-grid"><article v-for="account in connectableAccounts" :key="account.id" :class="`state-${readiness(account).step}`"><header><div class="account-avatar">{{ account.displayName.slice(0, 1) }}</div><div class="identity"><strong>{{ account.displayName }}</strong><span>{{ account.company.name }}</span></div><el-tag :type="readiness(account).type" effect="light">{{ readiness(account).label }}</el-tag></header><div class="progress-line"><span v-for="(label,index) in ['未连接','已连接','识别页面','可以使用']" :key="label" :class="{done:index+1<=readiness(account).step,current:index+1===readiness(account).step}"><b>{{ index+1<readiness(account).step ? '✓' : index+1 }}</b><small>{{ label }}</small></span></div><div class="next-action"><small>{{ readiness(account).step === 4 ? '当前状态' : '接下来怎么做' }}</small><strong>{{ accountHint(account) }}</strong></div><footer><span v-if="activeDevice(account.id)?.lastHeartbeatAt">最近连接 {{ formatDate(activeDevice(account.id)?.lastHeartbeatAt) }}</span><span v-else>尚未连接过</span><el-button type="primary" @click="openConnection(account)">{{ actionLabel(account) }}</el-button></footer></article></div></section>
