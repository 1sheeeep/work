...
- Flyway V1–V6 在 PostgreSQL 17 真实容器中迁移成功。
- 后端 Java 21 + Maven 编译成功，33/33 测试通过。
- 前端 TypeScript 检查和 Vite 标准生产构建通过。
- 前端 Vitest 6/6 通过。
- Playwright 组织、HR 用户、BOSS 账号、职位、招聘任务和候选人的桌面/移动回归 12/12 通过。
...
1. 进入 M7 “面试时间确认与 HR 通知”：确认候选人时区、可选时段、冲突/过期/重约状态机和 HR 通知渠道。
2. 实现 Flyway V7、面试记录、Mock Notification Gateway、通知幂等/重试和完整 V1 主链路验收。
