(()=>{
 ...
 async function tryFillApprovedDraft(config,snapshot){if(document.visibilityState!=='visible'||snapshot.direction!=='INBOUND')return;const empty=editorText(snapshot.editor).trim()==='';if(!empty)return pause('输入框已有内容，禁止覆盖 HR 草稿');const blocker=fillOnlyBlocker(snapshot);if(blocker)return pause(blocker);const claim=await send({type:'CLAIM_DRAFT_FILL',payload:{chatDigest:await digest(snapshot.chatIdentity||snapshot.chatId),messageDigest:await digest(snapshot.messageId)}});if(!claim?.ok||!claim.allowed)return;const verified=await stableSnapshot(config.selectors,config.stabilityDelayMs);if(!verified.ok||verified.messageKey!==snapshot.messageKey||verified.direction!=='INBOUND'||fillOnlyBlocker(verified)){await send({type:'DRAFT_FILL_RECEIPT',claimId:claim.claimId,payload:{status:'UNKNOWN'}});return pause('草稿填写前会话状态发生变化')};fillEditor(verified.editor,claim.content);const finalCheck=await readSnapshot(config.selectors);if(!finalCheck.ok||finalCheck.messageKey!==snapshot.messageKey||finalCheck.direction!=='INBOUND'||editorText(verified.editor).trim()!==claim.content.trim()){clearEditor(verified.editor);await send({type:'DRAFT_FILL_RECEIPT',claimId:claim.claimId,payload:{status:'UNKNOWN'}});return pause('草稿填写结果无法确认，已清空输入框')};await send({type:'DRAFT_FILL_RECEIPT',claimId:claim.claimId,payload:{status:'FILLED'}});await report(config,finalCheck,'READY','后台批准草稿已填入输入框，等待 HR 人工检查并发送',true)}
