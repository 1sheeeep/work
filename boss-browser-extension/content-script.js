(()=>{
 ...
 async function readSnapshot(s){
  ...
  try{active=firstVisible(s.activeConversation);identity=firstVisible(s.conversationIdentity);editor=s.editor?firstVisible(s.editor):null;sendButton=s.sendButton?firstVisible(s.sendButton):null;const items=[...(active?.querySelectorAll(s.message)||[])];last=items.filter(item=>directionOf(item,s)).at(-1)}catch{return{ok:false,reason:'页面选择器无效'}}
