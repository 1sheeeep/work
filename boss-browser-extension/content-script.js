(()=>{
 ...
 async function tick(){
  ...
  try{
   ...
   if(config.monitorOnly){const reason=`只监测模式：已稳定识别 ${list.total} 个会话，其中 ${list.unread} 个未读（${list.identityMode}）`;await report(config,null,'OBSERVING',reason);return heartbeat('PAUSED','只监测模式已开启，不会打开会话或发送消息')}
 ...
 async function report(config,snapshot,status,reason,bound){const adapterDigest=await digest(JSON.stringify(config.selectors||{})),list=conversationStats(config.selectors),messages=messageStats(config.selectors),payload={status,reason,adapterDigest,visible:document.visibilityState==='visible',bound,...list,...messages};if(snapshot){payload.chatDigest=await digest(snapshot.chatId);payload.messageDigest=await digest(snapshot.messageId);payload.direction=snapshot.direction;payload.createdAt=new Date(snapshot.createdAt).toISOString();payload.ageMinutes=(Date.now()-snapshot.createdAt)/60_000}return send({type:'DIAGNOSTIC',payload})}
