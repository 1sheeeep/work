...
const ready=[!['DEVICE_NOT_PAIRED','DEVICE_UNAUTHORIZED'].includes(response.backendState),response.backendState==='CONNECTED',Boolean(runtime.diagnostic?.status==='READY'||runtime.diagnostic?.status==='OBSERVING'),!config.emergencyStop]
document.querySelector('#state').innerHTML=`<div class="popup-state ${runtime.state==='RUNNING'?'ok':''}"><b>${escape(labels[runtime.state]||runtime.state)}</b><span>${escape(runtime.reason||'运行正常')}</span></div><div class="checklist">${['账号已连接','招聘系统在线','会话页面已识别','允许执行托管'].map((label,index)=>`<p class="${ready[index]?'done':''}"><i>${ready[index]?'✓':index+1}</i>${label}</p>`).join('')}</div><p class="quota">今日已接待 ${runtime.sentToday} / ${config.dailyLimit}</p>`
