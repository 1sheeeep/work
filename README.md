...
- V25 补齐 HR 手动发送闭环：选中会话的最新方向变为 HR 外发后，未读任务自动结束并区分“HR 已回复”和“草稿后由 HR 发送”；同一会话再收到新来信时建立新周期，不复用旧审核和租约
- V26 强制正式账号隔离：只有 `BROWSER_COMPANION` 账号可生成配对码、认证扩展和绑定网页会话；历史 Mock 设备与配对码自动失效，Mock 托管自动关闭，测试观察和记录不进入 HR 真实运行界面
