...
- 桥接器支持 HR 在 BOSS“职位管理”页手动触发当前可见职位同步：只读取具备稳定 DOM 标识的岗位名称、地点、薪资、经验、学历和可见职位描述，在页面内将平台 ID 转换为 SHA-256 摘要后上传。后端按招聘账号、来源摘要和规范化标题去重，只创建或更新待审核草稿；不会自动翻页、点击或修改 BOSS 职位。
...
同步岗位时，在对应 Profile 中手动打开 BOSS“职位管理”页，等待列表稳定后打开扩展并点击“同步当前职位页”。如岗位有分页，由 HR 手动切换页面并逐页同步；系统按账号和标题去重。需要完整资料时，由 HR 手动打开某个岗位的编辑详情页，等待表单稳定后再次点击同一按钮。招聘值守台严格按 BOSS 页面显示公司、招聘类型、职位名称、职位描述、职位类型、是否驻外、经验、学历、薪资详情、职位关键词和工作地址；不再提供手工新增职位或混入内部字段。扩展不会点击编辑、保存或发布。回到“岗位资料”点击“刷新同步结果”，展开岗位行即可查看详情；人工核对前不会启用或参与安全回复。
