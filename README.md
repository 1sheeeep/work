...
同步岗位时，在对应 Profile 中手动打开 BOSS“职位管理”页，等待列表稳定后打开扩展并点击“同步当前职位页”。如岗位有分页，由 HR 手动切换页面并逐页同步；系统按账号和标题去重。需要完整资料时，由 HR 手动打开某个岗位的编辑详情页，等待表单稳定后再次点击同一按钮，系统会补充招聘类型、职位描述、职位类别、驻外类型、经验、学历、薪资、薪数、关键词和详细地址。扩展不会点击编辑、保存或发布。回到招聘值守台“岗位资料”点击“刷新同步结果”，展开岗位行即可查看详情；人工核对前不会启用或参与安全回复。
